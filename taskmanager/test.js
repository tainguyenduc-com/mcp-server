...
async function main() {
  ...
  try {
    ...
    await runTest("Get non-existent task", async () => {
      ...
      assert(isError, "Should return error for non-existent task");
        ...
        // ── Test 13: Create Task with specific UUID and retrieve ──────────────
    await runTest("Create task with specific UUID and retrieve", async () => {
      const specificUuid = "a1b2c3d4-e5f6-4a7b-8c9d-1e0f2a3b4c5d";
      const createResponse = await client.call("tools/call", {
        name: "task_create",
        arguments: {
          id: specificUuid,
          title: "Task with specific UUID",
          description: "This task has a pre-defined UUID.",
        },
      });

      assert(!createResponse.result?.isError, "Should not return error for specific UUID creation");
      const createdTask = JSON.parse(createResponse.result.content[0].text);
      assert(createdTask.id === specificUuid, "Created task ID should match the specific UUID");

      const getResponse = await client.call("tools/call", {
        name: "task_get",
        arguments: { taskId: specificUuid },
      });

      assert(!getResponse.result?.isError, "Should not return error when getting task by specific UUID");
      const retrievedTask = JSON.parse(getResponse.result.content[0].text);
      assert(retrievedTask.id === specificUuid, "Retrieved task ID should match the specific UUID");
      assert(retrievedTask.title === "Task with specific UUID", "Retrieved task title should match");
    });

    // ── Test 14: Invalid UUID on create ──────────────────────────────────
    await runTest("Reject task creation with invalid UUID", async () => {
      const invalidUuid = "not-a-valid-uuid";
      const createResponse = await client.call("tools/call", {
        name: "task_create",
        arguments: {
          id: invalidUuid,
          title: "Task with invalid UUID",
        },
      });

      const responseBody = createResponse?.result;
      const isError = responseBody?.isError === true || 
        (responseBody?.content?.[0]?.text && 
         JSON.parse(responseBody.content[0].text).error);
      assert(isError, "Should return error for invalid UUID");
      assert(responseBody.content[0].text.includes("Invalid UUID format"), "Error message should indicate invalid UUID format");
    });
  ...
  // ── Additional Tests for Task Manager Fixes
    
    await runTest('CreateTask validation - empty title throws', async () => {
      const resp = await client.call('tools/call', {
        name: 'task_create',
        arguments: {title: '', description: 'test', priority: 'high'},
      });
      assert(resp.error?.message?.includes("title is required"), 'Should return error for empty title');
    });
    
    await runTest('CreateTask validation - invalid priority throws', async () => {
      const resp = await client.call('tools/call', {
        name: 'task_create',
        arguments: {title: 'Test', description: 'test', priority: 'invalid'},
      });
      const result = resp.result;
      assert(result?.isError === true, 'Should return error for invalid priority');
      assert(result.content[0].text.includes("Invalid priority"), "Error message should indicate invalid priority");
    });
    
    await runTest('CreateTask validation - valid input success', async () => {
      const resp = await client.call('tools/call', {
        name: 'task_create',
        arguments: {
          title: 'Valid Task',
      ...
      const result = resp.result;
      assert(result?.isError !== true, 'Should succeed for valid input');
      const task = JSON.parse(result.content[0].text);
      assert(task.title === 'Valid Task', 'Task title should match');
      assert(task.priority === 'medium', 'Priority should be medium');
    });
    
    await runTest('listTasks search - description case-insensitive', async () => {
      const resp = await client.call('tools/call', {name: 'task_list'});
      const tasks = JSON.parse(resp.result.content[0].text);
      // simple existence check
      assert(Array.isArray(tasks) && tasks.length > 0, 'Should return at least one task');
    });
    
    await runTest('listTasks search - search by tags', async () => {
      const resp = await client.call('tools/call', {name: 'task_list'});
      const tasks = JSON.parse(resp.result.content[0].text);
      const matches = tasks.filter(t => t.tags?.includes('writer-management'));
      assert(matches.length > 0, 'Should find task with matching tag');
    });
    
    await runTest('listTasks search - no match returns empty', async () => {
      const resp = await client.call('tools/call', {name: 'task_list'});
      const tasks = JSON.parse(resp.result.content[0].text);
      const matches = tasks.filter(t => t.title.includes('nonexistent'));
      assert(matches.length === 0, 'Should return empty array when no match');
    });
    
    await runTest('autoUpdateParentProgress loop guard', async () => {
      // Create two tasks with cyclic parent relationship
      const createA = await client.call('tools/call', {
        name: 'task_create', arguments: {title: 'Task A', priority: 'low'},
      });
      const createB = await client.call('tools/call', {
        name: 'task_create', arguments: {title: 'Task B', priority: 'low'},
      });
      const idA = JSON.parse(createA.result.content[0].text).id;
      const idB = JSON.parse(createB.result.content[0].text).id;
      // Set parent relationships cyclically
      await client.call('tools/call', {
        name: 'task_update', arguments: {
          taskId: idA, context: {parentTaskId: idB},
        },
      });
      await client.call('tools/call', {
        name: 'task_update', arguments: {
          taskId: idB, context: {parentTaskId: idA},
        },
      });
      // Trigger progress update which should not infinite loop
      await client.call('tools/call', {
        name: 'task_update', arguments: {taskId: idA, progress: 0},
      });
      // If we reach here without timeout, guard works
    });
    
    await runTest('task_server_info returns version, storePath, toolsCount', async () => {
      const resp = await client.call('tools/call', {name: 'task_server_info'});

      const info = JSON.parse(resp.result.content[0].text);
      assert(info?.version !== undefined, 'Should have version');
      assert(info?.storePath !== undefined, 'Should have storePath');
      assert(info?.toolsCount !== undefined, 'Should have toolsCount');
    });
    
    await runTest('task_stats returns correct counts by status and priority', async () => {
      const resp = await client.call('tools/call', {name: 'task_stats'});

      const stats = JSON.parse(resp.result.content[0].text);

      // Expect structure: {pending: {...}, in_progress: {...}, completed: {...}, failed: {...}, byPriority: {...}}
      assert(stats?.byStatus?.pending !== undefined, 'Should have pending counts');
      assert(stats?.byPriority?.high !== undefined, 'Should have in_progress high count');
      assert(stats?.byPriority?.low !== undefined, 'Should have completed low count');
    });
