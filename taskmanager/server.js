...
function autoUpdateParentProgress(taskId, visited = new Set()) {
  if (visited.has(taskId)) return; // Cycle detected
  visited.add(taskId);
...
Output: Task object vừa được tạo.`,
        inputSchema: {
          ...
          properties: {
            ...
            id: {
                type: "string",
                description: "ID tùy chỉnh cho task (nếu không có sẽ tự sinh UUID)"
            },
...
Output: { deletedCount: number } — number of files/folders deleted.`,
      ...
      {
        name: "task_stats",
        description: "Lấy thống kê tasks: tổng số, phân bổ theo status, phân bổ theo priority",
        inputSchema: { type: "object", properties: {} },
      },
    ],
  };
