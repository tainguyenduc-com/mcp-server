...
- **15 MCP tools** — Đầy đủ lifecycle: create, claim, update, report, pause, fail, cancel
...
| Tool | Description | Used By |
|------|-------------|---------|
| `task_list` | List tasks with filters (status, tag, parentTaskId, offset, ...) | Manager, lead |
...
| `task_stats` | Get task statistics (count by status, priority) | Manager, lead |
| `task_server_info` | Get server version, capabilities, store location | All |

### Server Info / `task_server_info`

Returns server metadata — version, capabilities, store location.
*Trả về metadata server — version, capabilities, vị trí store.*
...
```json
{
  "version": "1.0.0",
  "capabilities": [
    "create", "claim", "update", "report", "pause",
    "fail", "cancel", "list", "get", "find_available",
    "tree", "subtasks", "ancestors", "stats", "server_info"
  ],
  "storeLocation": "/path/to/.task/tasks.json",
  "taskCount": 42
}
```
...
const task = await task_create({
  ...
  context: { module: "jobs" },
  reportTo: "lead",
  createdBy: "project-manager",       // optional
  progress: 0                         // optional, default 0
});
// task.id contains the generated UUID
```
